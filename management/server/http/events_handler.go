...
func (h *EventsHandler) GetAllEvents(w http.ResponseWriter, r *http.Request) {
	...
	accountEvents, err := h.accountManager.GetEvents(r.Context(), claims.AccountId, claims.UserId)
	if err != nil {
	...
	err = h.fillEventsWithUserInfo(r.Context(), events, claims.AccountId, claims.UserId)
	if err != nil {
		util.WriteError(r.Context(), err, w)
		return
	}
